...
def create_complex_vector_animation(
    ...
    for i in range(num_frames):
        ...
        ax.set_title(r"$\mathbf{1A}$: Complex Plane")
        ax.set_aspect('equal')
        ...
        ax.set_title(r"$\mathbf{1B}$: Real Value of the Vector Over Time")
        ax.grid(True)
        ...
        ax.set_title(r"$\mathbf{1C}$: Imaginary Value of the Vector Over Time")
        ax.grid(True)
...
def create_complex_vector_and_freq_animation(
    ...
    for i in range(num_frames):
        ...
        ax.set_xlabel("Real")
        ax.set_ylabel("Imaginary")
        ax.set_title(r"$\mathbf{2A}$: Complex Plane")
        ax.set_aspect('equal')
        ...
        ax.set_xlabel("Frame")
        ax.set_ylabel("Real Value")
        ax.set_title(r"$\mathbf{2B}$: Real Value of the Vector Over Time")
        ax.grid(True)
        ...
        if show_phase_freq:
            ...
            ax1.set_title(r"$\mathbf{2C}$: Phase and Instantaneous Frequency Over Time")
...
def create_real_signal_and_positive_negative_complex_vectors_animation(
    ...
    for i in range(num_frames):
        ...
        # --- Top subplot: Real Value over Time ---
        ax = axs[0]
        ax.plot(range(i+1), real_vals[:i+1], 'r-')
        # Add a red dot at the most recent real value.
        ax.plot(i, real_vals[i], 'ro')
        ...
        ax.set_xlabel("Frame")
        ax.set_ylabel("Real Value")
        ax.set_title(r"$\mathbf{3A}$: Real Value of the Vector Over Time")
        ax.grid(True)

        # --- Bottom subplot: Complex Plane ---
        ax = axs[1]
        max_val = amplitude_base + abs(amplitude_mod) + 0.5
        ax.set_xlim(-max_val, max_val)
        ax.set_ylim(-max_val, max_val)
        ax.set_xlabel("Real")
        ax.set_ylabel("Imaginary")
        ax.set_title(r"$\mathbf{3B}$: Complex Plane")
        ax.set_aspect('equal')
        ...
        # --- Draw the vector as an arrow using quiver for better visibility ---
        ax.quiver(0, 0, vec.real*0.5, vec.imag*0.5, angles='xy', scale_units='xy', scale=1, color='black', width=0.01, headwidth=4, headlength=5)
        ax.quiver(vec.real*0.5, vec.imag*0.5, vec.real*0.5, -vec.imag*0.5, 
                  angles='xy', scale_units='xy', scale=1, color='gray', linestyle='dotted', width=0.005, headwidth=8, headlength=10)
        # --- Draw the vector as an arrow using quiver for better visibility ---
        ax.quiver(0, 0, vec.real*0.5, -vec.imag*0.5, angles='xy', scale_units='xy', scale=1, color='gray', width=0.01, headwidth=4, headlength=5)
        ax.quiver(vec.real*0.5, -vec.imag*0.5, vec.real*0.5, vec.imag*0.5, 
                  angles='xy', scale_units='xy', scale=1, color='black', linestyle='dotted', width=0.005, headwidth=8, headlength=10)

        # --- Added dots on the axes: ---
        ax.plot(vec.real, 0, 'ro')  # Red dot on the real axis.
        ax.plot(0, vec.imag*0.5, 'bo')  # Blue dot on the imaginary axis.

        # --- Added dots on the axes: ---
        #ax.plot(vec.real, 0, 'rd')  # Red dot on the real axis.
        ax.plot(0, -vec.imag*0.5, 'bd')  # Blue dot on the imaginary axis.

        # --- Add thin gray dotted lines for decomposition visualization ---
        # ax.plot([vec.real*0.5, vec.real*0.5], [0, vec.imag*0.5], 'gray', linestyle='dotted', linewidth=1)  # Vertical line
        ax.plot([0, vec.real*0.5], [vec.imag*0.5, vec.imag*0.5], 'gray', linestyle='dotted', linewidth=1)  # Horizontal line
...
if __name__ == '__main__':
    ...
    #                                 mod_rate=3,
    #                                 fps=6,
    #                                 show_phase_freq=False)
    create_real_signal_and_positive_negative_complex_vectors_animation("real_signal_and_positive_negative_complex_vectors.gif", 
                                    num_frames=120, # 120
                                    initial_phase_step=0.1,
                                    amplitude_base=1.0,
                                    amplitude_mod=0.5,
