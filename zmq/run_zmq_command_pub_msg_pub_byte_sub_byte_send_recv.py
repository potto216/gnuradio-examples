...
def main():
    ...
    port_number_pub_msg = 5555
    port_number_pub_data = 5556
    port_number_sub_data = 5557
    data_length = 200  # Configurable amount of data
    max_loop = 500
    
    # Socket to talk to server
    connection_url_msg = f"tcp://{ip_address}:{port_number_pub_msg}"
    connection_url_data = f"tcp://{ip_address}:{port_number_pub_data}"
    connection_url_sub = f"tcp://{ip_address}:{port_number_sub_data}"
    ...
    socket_pub_data = context.socket(zmq.PUB)
    socket_sub_data = context.socket(zmq.SUB)
    socket_msg.bind(connection_url_msg)
    socket_pub_data.bind(connection_url_data)
    socket_sub_data.connect(connection_url_sub)
    socket_sub_data.setsockopt_string(zmq.SUBSCRIBE, "")  # Subscribe to all topics
    
    amp_value = 1
    last_value = 0  # Initialize the last value
    
    # Open file to write received binary data as hex strings
    with open("received_data_cnt.txt", "w") as file:
        ...
        for request in range(max_loop):
            ...
            socket_pub_data.send(counting_sequence)
            ...
            try:
                binary_data = socket_sub_data.recv(flags=zmq.NOBLOCK)
    ...
    # Clean up
    socket_msg.close()
    socket_pub_data.close()
    socket_sub_data.close()
    context.term()
